...
- **Do not create technical-layer folders.** Avoid `src/services/`, `src/controllers/`, `src/repositories/`, `src/helpers/`, `src/utils/`, `src/lib/`. They scatter related code by *what it is* instead of *what it does*, which forces a full-tree grep to understand any single feature. If you find yourself reaching for `src/lib/foo.ts`, the right move is `src/<foo-domain>/foo.ts`.
  ...
  - **Exception — domain-local convention files.** The ban is on cross-cutting technical-layer *folders*, not on conventionally-named *files colocated inside a domain folder*. Within a domain (or a sub-domain folder), `types.ts`, `helpers.ts`/`utils.ts`, and `constants.ts` are fine — they keep the feature graspable from one folder while splitting a large module into type declarations, pure mappers, and the orchestrator that composes them. Example: `src/cv/export/json/{types.ts,helpers.ts,build-json-resume.ts}`. What stays banned is pulling that code *out* of its domain into a top-level `src/helpers/` or `src/utils/`.
