...
4. **Draft the description using this template.** Output as a single fenced markdown block ready to copy.
...
6. **Return only the markdown block.** Do not run `gh pr create`, do not push, do not commit. The user is copying the description by hand or invoking `gh pr create` separately.
